...
using static CevarnsOfEvil.GameConstants;
...
namespace CevarnsOfEvil
{
    ...
    public class EscapeToGui : MonoBehaviour
    {
        ...
        private void PauseForController(InputDevice device, InputDeviceChange change) {
            if(((change == InputDeviceChange.Disconnected) || (change == InputDeviceChange.Removed)) && (menu != null))
            {
                unplugged = true;
                menu.SetActive(true);
                playerAct.enabled = false;
                ...
                Time.timeScale = 0;
                Cursor.lockState = CursorLockMode.None;
            } else if (unplugged && ((change == InputDeviceChange.Reconnected) || (change == InputDeviceChange.Added))
                                 && (menu != null)) {
                unplugged = false;
                playerAct.usingItem = false;
                menu.GetComponent<ReturnToGame>().Return();
