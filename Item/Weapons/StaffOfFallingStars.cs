    ...
    public class StaffOfFallingStars : Wand
    {
        ...
        public override void OnPlayerUse(PlayerAct player, Animator anim)
        {
            if ((Charges > 0) && (Time.time > fireTime))
            {
                ...
                FireMeteorite(aim.from, aim.toward, player.PlayerScript);
                anim.SetTrigger("Act");
                launchSound.Play();
        ...
        public override void OnMobUse(Entity mob)
        {
            if (Time.time > fireTime)
            {
                ...
                aim.from = lauchSpawn.position;
                FireMeteorite(aim.from, aim.toward, mob);
                launchSound.Play();
            }
        }


        protected void FireMeteorite(Vector3 from, Vector3 toward, Entity attacker)
        {
            GameObject proj = Instantiate(meteoritePrefab, from, aimTransform.rotation);
